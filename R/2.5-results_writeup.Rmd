...
(*M* = `r round(S[S$Stimulus=="Conflict",2],2)`, *SE* = `r round(S_se["Conflict"],  2)`)
than to non-conflict trials
(*M* = `r round(S[S$Stimulus=="Non-conflict",2],2)`, *SE* = `r round(S_se["Non-conflict"],  2)`).
Accuracy was higher on session one
(*M* = `r round(sess[sess$Session=="One",2],2)`, *SE* = `r round(sess_se["One"],  2)`)
than on session two
(*M* = `r round(sess[sess$Session=="Two",2],2)`, *SE* = `r round(sess_se["Two"],  2)`)
...
(*M* = `r round(S_RT[S_RT$Stimulus=="Conflict",2],2)`, *SE* = `r round(S_se_RT["Conflict"],  2)`)
than to non-conflict trials
(*M* = `r round(S_RT[S_RT$Stimulus=="Non-conflict",2],2)`, *SE* = `r round(S_se_RT["Non-conflict"],  2)`). As displayed in Table 3, the effects of 
...
(*M* = `r round(sess_cond_RT %>% filter(Session=='One' & Condition=='Automation') %>% 
pull('mean(RT)'),2)`,
*SE* = `r round(sess_cond_se_RT["One", "Automation"],  2)`)
than in the manual condition
(*M* = `r round(sess_cond_RT %>% filter(Session=='One' & Condition=='Manual') %>% 
pull('mean(RT)'),2)`,
*SE* = `r round(sess_cond_se_RT["One", "Manual"],  2)`)
.
On session two, 
RTs were less substantially slower in the automation condition
(*M* = `r round(sess_cond_RT %>% filter(Session=='Two' & Condition=='Automation') %>% 
pull('mean(RT)'),2)`,
*SE* = `r round(sess_cond_se_RT["Two", "Automation"],  2)`)
than in the manual condition
(*M* = `r round(sess_cond_RT %>% filter(Session=='Two' & Condition=='Manual') %>% 
pull('mean(RT)'),2)`,
*SE* = `r round(sess_cond_se_RT["Two", "Manual"],  2)`), 
